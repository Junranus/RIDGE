...
class SGCN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, num_layers, node_num, lamb=5, bias=True):
        super().__init__()
        ...
        for i in range(num_layers):
            if i == 0:
                conv = SignedConv(in_channels, hidden_channels//2, first_aggr=True)
            else:
                conv = SignedConv(hidden_channels//2, hidden_channels//2, first_aggr=False)
    ...
    def encode(self, x, pos_edge_index, neg_edge_index):
        ...
        self.tmp_z = self.convs[-1](z, pos_edge_index, neg_edge_index)
        ...
        xs = 0
        for conv in self.convs[:-1]:
            x = conv(x, new_pos_index, new_neg_index).relu()
            xs += x
